...
public class TitleService implements IService<Title> {
    ...
    public void createPst(Title title) {
        ...
        try {
            ...
            // Retrieve the generated ID and set it in the Title object
            ResultSet generatedKeys = pst.getGeneratedKeys();
            if (generatedKeys.next()) {
                int generatedId = generatedKeys.getInt(1);
                title.setId(generatedId); // Set the ID in the Title object
            } else {
    ...
    public List<Title> readByUserId(int userId) {
        List<Title> titles = new ArrayList<>();
        String requete = "SELECT t.* FROM title t JOIN user_titles ut ON t.id = ut.title_id WHERE ut.user_id = ?";
        try {
            pst = cnx.prepareStatement(requete);
            pst.setInt(1, userId);
            rs = pst.executeQuery();
