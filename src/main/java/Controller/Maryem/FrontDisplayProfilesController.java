...
public class FrontDisplayProfilesController {
    ...
    private void openProfileDetailsWindow(Profile profile) {
        try {
            ...
            URL imageUrl = getClass().getResource("header.png"); // Adjust to your image path
            if (imageUrl != null) {
                Image headerImage = new Image(imageUrl.toExternalForm());
                headerImageView.setImage(headerImage);
            } else {
                System.err.println("Error: header_image.jpg not found");
                headerImageView.setImage(new Image("https://thumbs.dreamstime.com/b/stunning-hd-pic-caribbean-beach-cocktail-featuring-coconut-pineapple-set-against-palm-trees-sand-blue-sea-359953956.jpg")); // Fallback image
            }
            headerImageView.setFitWidth(1000); // Match previous WebView width
            headerImageView.setFitHeight(490); // Match previous WebView height
            headerImageView.setPreserveRatio(false); // Optional: Stretch to fit
