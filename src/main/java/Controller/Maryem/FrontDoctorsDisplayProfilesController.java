...
public class FrontDoctorsDisplayProfilesController {
    ...
    private void openProfileDetailsWindow(Profile profile) {
        try {
            ...
            URL imageUrl = getClass().getResource("header.png"); // Adjust the path to your image
            if (imageUrl != null) {
                Image headerImage = new Image(imageUrl.toExternalForm());
                headerImageView.setImage(headerImage);
            } else {
                // Fallback in case the image is not found
                headerImageView.setImage(new Image("https://thumbs.dreamstime.com/b/stunning-hd-pic-caribbean-beach-cocktail-featuring-coconut-pineapple-set-against-palm-trees-sand-blue-sea-359953956.jpg")); // Placeholder image
            }
            headerImageView.setFitWidth(2000); // Set width to match previous WebView
            headerImageView.setFitHeight(300); // Set height to match previous WebView
            headerImageView.setPreserveRatio(false); // Optional: Adjust if you want to stretch the image
